...
# URL target
BASE_URL = f"https://sipd-ri.kemendagri.go.id/renstra/{token}/?m=daerah_renstra_d_rakhir_manprokeg_beta&f=datatable_output"
...
async def main():
    ...
    cur.execute("SELECT * from renstra_kegiatan ORDER BY kodeskpd,kodeprogram,kodekegiatan")
    opd_list= cur.fetchall()
    
    for opd in opd_list:
        print(f"Memproses Program {opd['kodekegiatan']} - {opd['uraikegiatan']}")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0, cookies=COOKIES, headers=HEADERS, limits=connector) as client:
                ...
                sql = """
                        INSERT INTO renstra_output_kegiatan (
                            id, kodepemda, idperiode, kodebidang, kodeprogram, kodekegiatan,
                            idoutcome, urai, postdate, lastupdate, creator, updater,
                            kodeskpd, idparameter, total_indikator, total_subkegiatan,
                            total_catatan, pagu1, pagu2, pagu3, pagu4, pagu5, pagu6, pagu7,
                            urut, no, created_at, updated_at
                        ) VALUES %s;
                        """
                ...
                values = [
                            (
                                ...
                                r.get("kodeskpd"),
                                r.get("idparameter"),
                                int(r.get("total_indikator", 0) or 0),
                                int(r.get("total_subkegiatan", 0) or 0),
                                r.get("total_catatan", ""),
