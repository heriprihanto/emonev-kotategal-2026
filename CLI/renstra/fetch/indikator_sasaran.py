# This module maps to "indikator program" in original; name kept for parity
...
async def fetch_indikator_sasaran_for_program(p):
    url = f"{BASE}/renstra/{TOKEN}/?m=daerah_renstra_d_rakhir_manprokeg_beta&f=datatable_indikator_outcome_perangkat_daerah"
...
async def run(get_program_list):
    ...
    for p in items:
        try:
            n = await fetch_indikator_sasaran_for_program(p)
