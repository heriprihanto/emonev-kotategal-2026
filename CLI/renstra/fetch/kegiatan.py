...
async def fetch_kegiatan_for_program(p):
    payload = {
        "draw": "2",
        "start": "0",
        "length": "100000",
        ...
        "kodebidang": p.get("kodebidang", "")
    }
    url = f"{BASE}/renstra/{TOKEN}/?m=daerah_renstra_d_rakhir_manprokeg_beta&f=datatable_kegiatan"
    j = await post_with_retry(url, payload)
    xdata = json.dumps(j)
    await call_proc("insert_kegiatan", (xdata, p["kodeskpd"], p["idoutcome"], p["kodeprogram"], p.get("kodebidang", "")))
...
async def run(get_program_list):
    ...
    for p in items:
        try:
            n = await fetch_kegiatan_for_program(p)
