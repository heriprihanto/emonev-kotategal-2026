...
async def fetch_program_for_opd(opd):
    ...
    xdata = json.dumps(j)
    await call_proc("insert_program", (xdata, kodeskpd))
    return len(j.get("data", []))

async def run(get_subopd_list):
    opds = get_subopd_list()
    results = []
    for opd in opds:
        try:
            n = await fetch_program_for_opd(opd)
