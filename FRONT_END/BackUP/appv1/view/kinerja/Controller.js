...
Ext.define('Admin.view.kinerja.Controller', {
	...
	TwList_itemClick: function (grid, records) {
		...
		if (ROLE_ID_APP > 3) {
			...
			this.openSubPdList(vTAHUN, capk_id_pd, capk_current_tw);
	...
	updateFormNRecord: function (xIndex, xidindikator) {
		...
		Ext.Ajax.request({
			...
			success: function (response, opts) {
				...
				this.setTipeData();
				this.setStateFormIndikator(capk_current_tw);
				...
				tform.lookupName('tw').setValue(capk_current_tw);
			},
	...
	indikatorList_itemClick: function (grid, records) {
		...
		if (ROLE_ID === 11 || ROLE_ID === 12) {
			...
			const itemId = capk_current_tw -1;
	...
	openFormVerifikasiIndikator: function (pidindikator) {
		...
		/*
		const xformData = new Ext.Promise(function (resolve, reject) {
			Ext.Ajax.request({
				url: `${REMOTE_URL}kinerja/getindikator/${pidindikator}`, method: 'GET',
		...
		*/
		/*
		console.log(JSON.parse(xformData));
	...
	formPrint_Show: function (btn) {
		...
		if (xmode == 'opd') {
			xForm.lookupName('report_name').setOptions(
				[	
					{ text: '1. Capaian Kinerja PD', value: 'capaian_kinerja_pd', ispd: 1 },
					{ text: '2. Evaluasi Hasil Renja PD (Form E.81)', value: 'eval_renja', ispd: 1 },
					{ text: '3. Surat Pengantar Laporan', value: 'pengantar_eval_renja' },
	...
	formKirim_Show: function (btn) {
		var view = this.getView(),
			dialogSubmitLaporan = this.dialogSubmitLaporan;

		if (!dialogSubmitLaporan) {
			dialogSubmitLaporan = Ext.apply({
				ownerCmp: view
			}, view.dialogSubmitLaporan);

			this.dialogSubmitLaporan = dialogSubmitLaporan = Ext.create(dialogSubmitLaporan);
		}
		dialogSubmitLaporan.show();
		
		this.getFetchData(`${REMOTE_URL}kinerja/getformsubmit/${capk_id_sub_pd}/${capk_current_tw}`,{method:'GET'}).then(function (data) {
			var obj = JSON.parse(data);
			console.log(obj[0])
			var tform = dialogSubmitLaporan.down('formpanel'); 
			tform.lookupName('jml_indikator').setValue(Number(obj[0].jm_indikator));
			tform.lookupName('jml_kosong').setValue(Number(obj[0].jm_indikator_null));
			tform.lookupName('id_sub_pd').setValue(capk_id_sub_pd);
			tform.lookupName('tw').setValue(capk_current_tw);
	...
	uploadFile: function (t, n, o, e) {
		...
		xForm.submit({
			...
			failure: function (form, o) {
				var ermsg = JSON.parse(o.responseText);
				console.log(o);
				//this.toastPesan('error', 'Gagal upload berkas!<br/><br/>' + ermsg.detail[0].msg, 2000);
				this.toastPesan('error','Gagal upload file!',2000);
			}
		});
	...
	formLapTw_Show: function (btn) {
		...
		//form.lookupName('id_pd').setValue(capk_id_pd);
		form.lookupName('id_sub_pd').setValue(capk_id_sub_pd);
		...
		Ext.Msg.confirm('Konfirmasi', 'Apakah anda akan menghapus laporan ?',
			function (answer) {
				if (answer === 'yes') {
					...
					Ext.Ajax.request({
						url: `${REMOTE_URL}kinerja/deletelaporan/${capk_id_laporan}`,
						method :'DELETE',
						scope : this,
						success: function(response, opts) {
							var obj = Ext.decode(response.responseText);
