Ext.define('Admin.base.ViewController', {
	...
	getVarData: function () {
        return this.getViewModel().get('tempData');
    },

    setVarData: function (data) {
        this.getViewModel().set('tempData', Ext.apply(this.getVarData(), data));
    },
	setState(key, val) {
        this.state[key] = val;
    },
    ...
    switchTab: function (xThis,tabName,idx) {
        xThis.lookup(tabName).setActiveItem(idx);
    },

    updateHeader: function (xThis,vmName,text) {
        xThis.getViewModel().set(vmName, text);
    },
