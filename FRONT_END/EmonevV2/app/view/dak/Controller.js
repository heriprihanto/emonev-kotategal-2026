...
Ext.define('Admin.view.dak.Controller', {
	...
	prevRecord: function () {
		nxRecord = this.getStore('kegiatanStore').getAt(idxDakKegiatanStore - 1);
		...
		if (xForm.validate()) {
			xForm.submit({
				...
				success: function (f, r, d) {
					this.getStore('kegiatanStore').reload();
					this.toastPesan('success', 'Data Berhasil Disimpan  !!', 2000);
					//xDiag.hide();
					//xThis.nextRecord();
				},
				failure: function (form, o) {
					this.toastPesan('error', 'Gagal simpan data ' + o.error.message, 2000);
				}
			});
		} else {
