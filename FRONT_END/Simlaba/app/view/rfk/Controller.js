	...
	simpanKirimLaporan: function (btn) {
		...
		if (form.validate()) {
			form.submit({
				...
				failure: function (form, o) {
					var ermsg= JSON.parse(o.responseText);
					JTH.toastPesan('error','Terjadi Kesalahan<br/><br/>'+ermsg.detail[0].msg,2000);	
				}
		...
		if (xForm.validate()) {
			xForm.submit({
				...
				failure: function (form, o) {
					JTH.genCaptcha();
					var ermsg= JSON.parse(o.responseText);
					Ext.Msg.alert('Error', '<span class="badge2 bg-danger">Gagal sinkronisasi data dengan SIPD !! kemungkinan disebabkan : <br>- Username / Password Salah<br>- Aplikasi sedang offline <br>- Terjadi error pada aplikasi</span>');
				}
			});
		} else {
